...
const defItems = [defBlog];

app.get("/", function(req,res){
  Blog.find({},function(err,fitems){
    if(fitems.length==0){
      Blog.insertOne(defBlog,function(err){
        if(err){
          console.log(err);
        }
        else {
          console.log("Def items added successfully");
        }
      })
      res.redirect("/");
    }
    else {
        res.render("home",{homecontent : homeStartingContent, postsVal: fitems});
    }
  })
  // console.log(posts);
})
